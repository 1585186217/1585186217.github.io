...
//创建一个浮动的div，未指定宽高度及位置，指定部分样式
//var timer = null;
function CreateStateDivHall(divID) {
    var div = document.createElement("div");
    div.id = divID;
    /*
    if (tag) {
    ...
    */
    with (div.style) {
        ...
        zIndex = "100000";
        display = "none";
        //background = "url(/Images/System/d_bottom.gif) repeat-x"
    }
    //}
    return div;
}
...
function MsgHall(text, width) {
    ...
    var state = $("#Tranning")[0];
    if (state == null) {
        state = CreateStateDivHall("Tranning");
        AppendElement(state);
    }
    var swidth = (width == null || width == "" || width == 0) ? 600 : width;

    var content = '<div style="background:url(/Images/1gm_01.png) no-repeat; height:140px;width:189px;"><div style="cursor:move;" onmouseover="DivMove(\'Tranning\');">';
    content += '<span style="position: absolute; top: 10px; right:20px;">';
    content += '<img src="/Images/MembersImg/x.png" width="21" height="19" style="border: 0;cursor:pointer;" title="关闭" alt="关闭" onclick="HiddenState(\'Mask\', 1);HiddenState(\'Tranning\',200);" /></span>';
    content += '</div>';
    content += '<div style="padding: 55px 20px 0 20px; line-height: 20px;color:#e7b76b;">';
    content += text;
    content += '</div>';
    content += '<div style="height: 25px;text-align: center;position:absolute;top:100px;left:60px;">';
    content += '<img src="/Images/1gm_03.gif" style="border:0;cursor:pointer;" title="关闭" alt="关闭" onclick="HiddenState(\'Mask\', 1);HiddenState(\'Tranning\',200);" />';
    content += '</div></div>';
