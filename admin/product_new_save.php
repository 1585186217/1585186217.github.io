<?php
	...
	//3、构造SQL语句，写入数据表，实现新增产品功能  数字不用''
	$sql = "insert into product(productname,pro_no,cate_id,content,img) values('$productname','$pro_no',$cate_id,'$content','$filename')"; //图片保存的是文件名
	// echo $sql;
	$res = mysqli_query($conn,$sql);

	// //4、给出执行结果
	if($res){
		alert('新增产品成功','product_list.php');
	}else{
		alert('新增产品失败','product_new.php');
	}
